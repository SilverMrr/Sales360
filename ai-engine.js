(() => {
// ============================================================
// Sales360 — AI Engine (frontend-only, multi-provider)
// ============================================================
...
function getActiveAiSummary() {
  ...
  return `APIs séparées : ${audioLabel} + OpenAI CRM`;
}
...
async function transcribeAudioRaw(audioBlob) {
  const cfg = getAiConfig();
  if (cfg.mode === AI_MODE_GEMINI) return transcribeWithGemini(cfg.geminiKey, cfg.geminiAudioModel, audioBlob);
